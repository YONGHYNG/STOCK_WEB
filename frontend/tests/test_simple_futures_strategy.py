...
class StrategyTests(unittest.TestCase):
    ...
    def test_volume_breakout_enters_long_and_short_without_rsi_reclaim(self):
        long_frame = strategy_frame("LONG")
        long_frame.loc[218, "adx14"] = 24.0
        long_frame.loc[219, ["low", "high", "close", "rsi14"]] = [
            105.0, 112.0, 111.0, 70.0
        ]
        long_frame.loc[219, "volume_ratio"] = 3.0
        long_frame.loc[219, "adx14"] = 30.0
        long_decision = VolumeTrendRsiStrategy().evaluate(long_frame)
        self.assertEqual(long_decision.signal, "LONG_VOLUME_BREAKOUT")
        self.assertEqual(long_decision.direction, "LONG")

        short_frame = strategy_frame("SHORT")
        short_frame.loc[218, "adx14"] = 24.0
        short_frame.loc[219, ["low", "high", "close", "rsi14"]] = [
            88.0, 95.0, 89.0, 30.0
        ]
        short_frame.loc[219, "volume_ratio"] = 3.0
        short_frame.loc[219, "adx14"] = 30.0
        short_decision = VolumeTrendRsiStrategy().evaluate(short_frame)
    ...
    def test_volume_breakout_blocks_entry_beyond_point_four_atr(self):
        ...
        frame.loc[219, ["low", "high", "close", "rsi14"]] = [
            110.0, 125.0, 121.0, 75.0
        ]
        frame.loc[219, "volume_ratio"] = 3.0
        frame.loc[219, "adx14"] = 30.0
